...
void writeText(RenderWindow& window, string name, Font& mainFont,float fontSize, int horizontal, int vertical, int width, int height, bool bold, Color color) {
    ...
    if (horizontal == desktopsize.width) {
        text.setPosition((horizontal / 2.0) + width, vertical + height / 2.0);
    }
    else if (vertical == desktopsize.height) {
        text.setPosition(horizontal + width / 2.0, (vertical / 2.0) + height);
    }
    else
        text.setPosition(horizontal , vertical );
...
FloatRect makeButtons(RenderWindow& window, Font& mainFont, float fontSize, Color fontColor, string name, int width, int height, int horizontal, int vertical, Color buttonColor = Color::White, Color outLineColor = Color::Black, Color hoverColor = Color::White, Color hoverText = Color::Black) {
    ...
    width *= desktopsize.width / 1920.0;
    height *= desktopsize.height / 1080.0;
    horizontal *= desktopsize.width / 1920.0;
    vertical *= desktopsize.height / 1080.0;

    Vector2f mousePos = window.mapPixelToCoords(Mouse::getPosition(window));

    RectangleShape button(Vector2f(width, height));
    button.setOrigin(Vector2f((button.getGlobalBounds().left + button.getGlobalBounds().width) / 2, (button.getGlobalBounds().top + button.getGlobalBounds().height) / 2));
